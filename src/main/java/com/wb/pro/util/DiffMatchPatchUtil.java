...
/**
 * https://github.com/sksamuel/google-diff-match-patch/blob/master/src/main/java/com/sksamuel/diffpatch/DiffMatchPatch.java
 *
 * @author wangbin
...
public class DiffMatchPatchUtil {
    ...
    public static String linkedListDiff2HtmlString(LinkedList<Diff> diffs) {
        if (diffs == null || diffs.isEmpty()) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();
        for (Diff diff : diffs) {
            buffer.append(diff.htmlString());
        }
    ...
    private LinkedList<Diff> diffCompute(String text1, String text2,
        ...
        if (hm != null) {
            ...
            String aText = hm[0];
            String bText = hm[1];
            String aSecondText = hm[2];
            String bSecondText = hm[3];
            String midCommon = hm[4];
            // Send both pairs off for separate processing.
            LinkedList<Diff> diffsa = diffMain(aText, aSecondText,
                    checklines, deadline);
            LinkedList<Diff> diffsb = diffMain(bText, bSecondText,
                    checklines, deadline);
            // Merge the results.
            diffs = diffsa;
    ...
    protected LinkedList<Diff> diffBisect(String text1, String text2,
        ...
        int textFirstLength = text1.length();
        int textSecondLength = text2.length();
        int maxd = (textFirstLength + textSecondLength + 1) / 2;
        int vOffset = maxd;
        int vLength = 2 * maxd;
        ...
        v1[vOffset + 1] = 0;
        v2[vOffset + 1] = 0;
        int delta = textFirstLength - textSecondLength;
        ...
        for (int d = 0; d < maxd; d++) {
            ...
            for (int k1 = -d + k1start; k1 <= d - k1end; k1 += 2) {
                ...
                int x1;
                boolean flag = k1 == -d || (k1 != d && v1[k1Offset - 1] < v1[k1Offset + 1]);
                if (flag) {
                    x1 = v1[k1Offset + 1];
                } else {
                    x1 = v1[k1Offset - 1] + 1;
                ...
                int y1 = x1 - k1;
                while (x1 < textFirstLength && y1 < textSecondLength
                        && text1.charAt(x1) == text2.charAt(y1)) {
                    x1++;
                    y1++;
                }
                v1[k1Offset] = x1;
                if (x1 > textFirstLength) {
                    // Ran off the right of the graph.
                    k1end += 2;
                } else if (y1 > textSecondLength) {
                    ...
                    if (k2Offset >= 0 && k2Offset < vLength && v2[k2Offset] != -1) {
                        ...
                        int x2 = textFirstLength - v2[k2Offset];
                        if (x1 >= x2) {
                            // Overlap detected.
            ...
            for (int k2 = -d + k2start; k2 <= d - k2end; k2 += 2) {
                ...
                int x2;
                boolean flag = k2 == -d || (k2 != d && v2[k2Offset - 1] < v2[k2Offset + 1]);
                if (flag) {
                    x2 = v2[k2Offset + 1];
                } else {
                    x2 = v2[k2Offset - 1] + 1;
                ...
                int y2 = x2 - k2;
                while (x2 < textFirstLength && y2 < textSecondLength
                        && text1.charAt(textFirstLength - x2 - 1)
                        == text2.charAt(textSecondLength - y2 - 1)) {
                    x2++;
                    y2++;
                }
                v2[k2Offset] = x2;
                if (x2 > textFirstLength) {
                    // Ran off the left of the graph.
                    k2end += 2;
                } else if (y2 > textSecondLength) {
    ...
    protected LinesToCharsResult diffLinesToChars(String text1, String text2) {
        ...
        Map<String, Integer> lineHash = new HashMap<String, Integer>(16);
    ...
    private String[] diffhalfmatchi(String longtext, String shorttext, int i) {
        ...
        int j = -1;
        String bestCommon = "";
        String bestLongtextFirst = "", bestLongtextSecond = "";
        String bestShorttextFirst = "", bestShorttextSecond = "";
        while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
            ...
            if (bestCommon.length() < suffixLength + prefixLength) {
                ...
                bestLongtextFirst = longtext.substring(0, i - suffixLength);
                bestLongtextSecond = longtext.substring(i + prefixLength);
                bestShorttextFirst = shorttext.substring(0, j - suffixLength);
                bestShorttextSecond = shorttext.substring(j + prefixLength);
            }
        }
        if (bestCommon.length() * 2 >= longtext.length()) {
            return new String[]{bestLongtextFirst, bestLongtextSecond,
                    bestShorttextFirst, bestShorttextSecond, bestCommon};
        } else {
            return null;
        }
    }
